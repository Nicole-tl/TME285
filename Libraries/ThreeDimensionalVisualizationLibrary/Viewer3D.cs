...
namespace ThreeDimensionalVisualizationLibrary
{
    public class Viewer3D: GLControl
    {
        ...
        public event EventHandler CameraPoseChanged = null;
        ...
        // 20191101 (note y <=> z, z <=> -y)
        public void SetCameraPosition(Vector3 cameraPosition)
        {
            cameraDistance = Math.Sqrt(cameraPosition[0] * cameraPosition[0] + cameraPosition[1] * cameraPosition[1] +
                                       cameraPosition[2] * cameraPosition[2]);
            cameraLongitude = Math.Atan2(cameraPosition[1], cameraPosition[0]);
            if (cameraPosition[2] >= 0) { cameraLatitude = Math.Asin(cameraPosition[2] / cameraDistance); }
            else { cameraLatitude = -Math.Asin(-cameraPosition[2] / cameraDistance); }
         //   cameraLatitude = Math.Acos(cameraPosition[2] / cameraDistance); //  -Math.PI/2;
            ApplyCameraPose(); 
        }
        ...
        {
            if (CameraPoseChanged != null)
            {
                EventHandler handler = CameraPoseChanged;
                handler(this, EventArgs.Empty);
            }
        }
        ...
        public Vector3 GetCameraPosition()
        {
            ...
            float cameraY = (float)(cameraDistance * Math.Sin(cameraLongitude) * Math.Cos(cameraLatitude));
            float cameraZ = (float)(cameraDistance * Math.Sin(cameraLatitude));
            Vector3 currentCameraPosition = new Vector3(cameraX, cameraY, cameraZ);
        ...
        //    GL.TexParameter(TextureTarget.Texture2D, (TextureParameterName)ExtTextureFilterAnisotropic.TextureMaxAnisotropyExt, maxAniso);  // TEST20191004

            GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.Linear);
            //   GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.LinearMipmapLinear);  // TEST20191004
            ...
            GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, textureBitmap.Width, textureBitmap.Height, 0,
                OpenTK.Graphics.OpenGL.PixelFormat.Bgra, PixelType.UnsignedByte, textureBitmapData.Scan0);  // TEST20191004
        ...
        private void AnimationLoop()
        {
            while (animationRunning)
            {
                ...
                if (!animationRunning) { break; } // Needed, in case the user exists the program during the sleep interval.
                if (Monitor.TryEnter(lockObject, 1))  // ToDo: Parameterize timeout.
                {
        ...
        // 20191024
        public void InterpolateKeyFrames(int objectIndex, int keyFrameIndex1, int keyFrameIndex2, double beta)
        {
            Monitor.Enter(lockObject);
            this.Scene.ObjectList[objectIndex].InterpolateKeyFrames(keyFrameIndex1, keyFrameIndex2, beta);
            Monitor.Exit(lockObject);
        }
        ...
        public Scene3D Scene
        {
            ...
            { 
                ...
                if (scene != null)
                {
                    scene.SetLockObject(lockObject); // Required for animations.
                    ...
                    {
                        cameraTarget[0] = scene.CameraTarget[0];
                        cameraTarget[1] = scene.CameraTarget[1];
                        cameraTarget[2] = scene.CameraTarget[2];
                    }
                    ApplyCameraPose();
                }
                else
        ...
        public System.Boolean UseSmoothShading
        {
            ...
            set
            {
                ...
                if (useSmoothShading != oldValue)
                {
                    List<Object3D> allObjects = scene.GetAllObjects(null, true);
                    foreach (Object3D object3D in allObjects) { object3D.ShadingModel = shadingModel; }
                    Invalidate();
                }
            }
        }
