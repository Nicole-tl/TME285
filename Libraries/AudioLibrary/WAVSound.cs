...
namespace AudioLibrary
{
    ...
    public class WAVSound
    {
        ...
        // 20190407
        public void GenerateFromListDouble(List<double> doubleSamplesList)
        {
            List<short> singleChannelSampleList = new List<short>();
            for (int jj = 0; jj < doubleSamplesList.Count; jj++)
            {
                singleChannelSampleList.Add((Int16)Math.Round(doubleSamplesList[jj]));
            }
            List<List<short>> sampleList = new List<List<short>>();
            for (int ii = 0; ii < numberOfChannels; ii++)
            {
                sampleList.Add(singleChannelSampleList);
            }
            GenerateFromSamples(sampleList);
        }
        ...
        // 20180116
        public List<double> GetSamplesAsListDouble(int iChannel)
        {
            List<double> doubleSampleList = new List<double>();
            for (int ii = 0; ii < samples[iChannel].Count; ii++)
            {
                Int16 sample = samples[iChannel][ii];
                doubleSampleList.Add((double)sample);
            }
            return doubleSampleList;
        }

        // Returns a value in the range 0 to 1, namely the top volume relative
        // to the maximum top volume (32768 for 16-bit sounds).
        public double GetTopVolume()
        {
            ...
            for (int iChannel = 0; iChannel < samples.Count; iChannel++)
            {
                ...
                {
                    int sample = Math.Abs((int)samples[iChannel][jj]);
                    if (sample > maximumSample) { maximumSample = sample; }
                }
            }
            double topVolume = (double)maximumSample / (double)32768;
            return topVolume;
        }

        // topVolume must be in the range 0 to 1.
        public void SetTopVolume(double topVolume)
        {
            SetMaximumNonClippingVolume();
            for (int iChannel = 0; iChannel < samples.Count; iChannel++)
            {
                for (int jj = 0; jj < samples[iChannel].Count; jj++)
                {
                    int sample = (int)samples[iChannel][jj];
                    Int16 sampleAtDesiredVolume = (Int16)Math.Truncate(sample * topVolume);
                    samples[iChannel][jj] = sampleAtDesiredVolume;
                }
            }
        ...
        // 20180119
        public double GetAverageAbsoluteVolume()
        {
            int iChannel = 0;
            double absoluteAverage = 0;
            for (int ii = 0; ii < samples[iChannel].Count; ii++)
            {
                absoluteAverage += (Math.Abs((double)samples[iChannel][ii]));
                //  absoluteAverage += (double)(Math.Abs(samples[iChannel][ii]));
            }
            absoluteAverage /= (samples[iChannel].Count);
            return absoluteAverage;
        }

        // 20180119
        public void SetAverageAbsoluteVolume(double volume)
        {
            double averageAbsoluteVolume = GetAverageAbsoluteVolume();
            double ratio = volume / averageAbsoluteVolume;
            for (int iChannel = 0; iChannel < numberOfChannels; iChannel++)
            {
                for (int ii = 0; ii < samples[iChannel].Count; ii++)
                {
                    double sampleBefore = samples[iChannel][ii];
                    double sampleAfter = ratio * sampleBefore;
                    if (Math.Abs(sampleAfter) < 32767)
                    {
                        samples[iChannel][ii] = (Int16)Math.Round(sampleAfter);
                    }
                    else
                    {
                        if (sampleAfter < 0) { samples[iChannel][ii] = MINIMUM_SAMPLE; }
                        else { samples[iChannel][ii] = MAXIMUM_SAMPLE; }
                    }
                }
            }
            GenerateSoundDataFromSamples();
        ...
        // 20171230
        public static WAVSound Fuse(List<WAVSound> soundList, List<double> weightList, Boolean preventClipping)
        {
            if (soundList == null) { return null; }
            if (soundList.Count == 0) { return null; }
            short numberOfChannels = soundList[0].NumberOfChannels;
            WAVSound fusedSound = new WAVSound("FusedSound", soundList[0].SampleRate, numberOfChannels, soundList[0].BitsPerSample);
            List<List<double>> doubleSampleListList = new List<List<double>>();
            double maximumAbsoluteSample = 0;

            int minimumNumberOfSamples = soundList[0].Samples[0].Count;
            for (int kk = 1; kk < soundList.Count; kk++)
            {
                int numberOfSamples = soundList[kk].Samples[0].Count;
                if (numberOfSamples < minimumNumberOfSamples) { minimumNumberOfSamples = numberOfSamples; }
            }

            for (int iChannel = 0; iChannel < numberOfChannels; iChannel++)
            {
                List<double> doubleSampleList = new List<double>();
                for (int ii = 0; ii < minimumNumberOfSamples; ii++)
                {
                    double newDoubleSample = 0;
                    for (int jj = 0; jj < soundList.Count; jj++)
                    {
                        newDoubleSample += weightList[jj] * soundList[jj].Samples[iChannel][ii];
                    }
                    double absoluteSample = Math.Abs(newDoubleSample);
                    if (absoluteSample > maximumAbsoluteSample) { maximumAbsoluteSample = absoluteSample; }
                    doubleSampleList.Add(newDoubleSample);
                }
                doubleSampleListList.Add(doubleSampleList);
            }
            List<List<Int16>> sampleListList = new List<List<Int16>>();
            double scaleFactor = 1;
            if (preventClipping)
            {
                if (maximumAbsoluteSample > 32767)
                {
                    scaleFactor = 32767 / maximumAbsoluteSample;
                }
            }
            for (int iChannel = 0; iChannel < numberOfChannels; iChannel++)
            {
                ...
                for (int ii = 0; ii < minimumNumberOfSamples; ii++)
                {
                    doubleSampleListList[iChannel][ii] *= scaleFactor;
                    Int16 sample = (Int16)Math.Round(doubleSampleListList[iChannel][ii]);
                    sampleList.Add(sample);
                }
                sampleListList.Add(sampleList);
            }
            fusedSound.GenerateFromSamples(sampleListList);
            //   fusedSound.SubtractMean();
            return fusedSound;
        }


        // 20190405
        public void GenerateFromMemoryStream()
        {
            soundData = new byte[wAVMemoryStream.Length];
            wAVMemoryStream.Position = 0;
            wAVMemoryStream.Read(soundData, 0,(int)wAVMemoryStream.Length);
            wAVMemoryStream.Close();
            ExtractInformation();
        }
        ...
        {
            double originalAverageEnergy = GetAverageEnergy(0);
            double adjustmentRatio = Math.Sqrt(averageEnergy / originalAverageEnergy);
            for (int iChannel = 0; iChannel < samples.Count; iChannel++)
            {
                for (int jj = 0; jj < samples[iChannel].Count; jj++)
                {
                    double newDoubleSample = Math.Truncate(adjustmentRatio * samples[iChannel][jj]);
        ...
        // 20190412  (Assumes single-channel sound, perhaps improve?)
        public static WAVSound Overlap(WAVSound sound1, WAVSound sound2)
        {
            List<double> overlappedSamplesList = new List<double>();
            WAVSound overlappedSound = new WAVSound("", sound1.sampleRate, sound1.NumberOfChannels, sound1.BitsPerSample);
            if (sound1.Samples[0].Count > sound2.Samples[0].Count)
            {
                
                foreach (Int16 sample in sound1.Samples[0])
                {
                    overlappedSamplesList.Add(sample);
                }
                for (int jj = 0; jj < sound2.Samples[0].Count; jj++)
                {
                    overlappedSamplesList[jj] += sound2.Samples[0][jj];
                }
            }
            else
            {
                foreach (Int16 sample in sound2.Samples[0])
                {
                    overlappedSamplesList.Add(sample);
                }
                for (int jj = 0; jj < sound1.Samples[0].Count; jj++)
                {
                    overlappedSamplesList[jj] += sound1.Samples[0][jj];
                }
            }
            overlappedSound.GenerateFromListDouble(overlappedSamplesList);
            return overlappedSound;
        }
        ...
        // 20190513
        public static void PlaySoundSync(WAVSound sound)
        {
            sound.GenerateMemoryStream();
            sound.WAVMemoryStream.Position = 0;
        ...
        // 20191113
        public static void PlaySoundAsync(WAVSound sound)
        {
            sound.GenerateMemoryStream();
            sound.WAVMemoryStream.Position = 0;
            using (SoundPlayer soundPlayer = new SoundPlayer())
