...
namespace AudioLibrary.Visualization
{
    public partial class SoundVisualizer : HorizontalScrollableZoomControl
    {
        ...
        private List<HighLight> highLightList = null;
        ...
        protected override void OnPaintBackground(PaintEventArgs e)
        {
            ...
            if (highLightList != null)
            {
                ...
                {
                    DrawHighLight(e.Graphics, highLight);
                }
            }
            if (selecting) { DrawSelection(e.Graphics); }
        }

        protected void DrawHighLight(Graphics g, HighLight highLight)
        {
            using (SolidBrush highLightBrush = new SolidBrush(Color.Empty))
            {
                highLightBrush.Color = Color.FromArgb(highLight.Opacity, highLight.Color);
                float yPlotStart = 0f;
                float yPlotEnd = this.Height;
                float xPlotStart = GetPlotXAtX(highLight.Start);
                float xPlotEnd = GetPlotXAtX(highLight.End);
                g.FillRectangle(highLightBrush, new RectangleF(xPlotStart, yPlotStart, xPlotEnd - xPlotStart, yPlotEnd - yPlotStart));
            }
        }
        ...
        {
            get { return highLightList; }
            set { highLightList = value; }
        }
